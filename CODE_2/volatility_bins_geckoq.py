...
def main():
    filepath = path.relpath(f'data/geckoq_all')
    geckoq_data = pd.read_csv(f'{filepath}/geckoq_molar_mass.csv')
    temp_geckoq = 288.15 # K
    geckoq_data['c'] = geckoq_data.apply(lambda x: \
            log_psat_to_c(x['log_p_sat'], x['molar_mass'], temp_geckoq), axis=1)
    geckoq_data['volatility'] = geckoq_data.apply(lambda x: \
                                        c_to_volatility_group(x['c']), axis=1)
    geckoq_data.to_csv(f'{filepath}/geckoq_data.csv', index=None)
    ##############
    geckoq_data = pd.read_csv(f'{filepath}/geckoq_data.csv', index_col='SMILES')
    filepath_preds = path.relpath(f'data/KRR_output/maccs and simpol final model')
    random_state = [12,5,7543,12343,452,325432435,326,436,2435]
    pred_all = pd.DataFrame()
    for seed in random_state:
        filename_preds = f'output_predictions_MACCS_with_simpol_geckoq_log_p_sat_{seed}.csv'
        pred = pd.read_csv(f'{filepath_preds}/{filename_preds}', index_col='SMILES')
        pred = pred.merge(geckoq_data,
                          left_on=['SMILES','target_values'], 
                          right_on = ['SMILES','log_p_sat'])
        pred['c_pred'] = pred.apply(lambda x:\
            log_psat_to_c(x['predictions'], x['molar_mass'], temp_geckoq), axis=1)
        pred['volatility_pred'] = pred.apply(lambda x: c_to_volatility_group(x['c_pred']), axis=1)
        pred.columns = ['log_p_sat_pred', 'target_value', 'molar_mass', 'log_p_sat',\
                         'sat_mass_c', 'volatility', 'sat_mass_c_pred', 'volatility_pred']
        reordered = ['molar_mass', 'log_p_sat', 'target_value', 'log_p_sat_pred',\
                      'sat_mass_c', 'sat_mass_c_pred', 'volatility', 'volatility_pred']
    ...
    wrong_preds.to_csv(f'{filepath}/wrong_predictions_geckoq_log_p_sat.csv')
    ...
    ax.plot(vol_donahue[0] * np.ones(100), np.linspace(-15,10,num=100), color='black')
    ax.fill_between(np.linspace(plt_real.min(), vol_donahue[0], num=100),
                    np.ones(100) * 2* plt_real.min(), np.ones(100) * 2\
                    * plt_real.max(), alpha=0.5, color='purple')
    for i, vol in enumerate(vol_donahue[1:]):
        ...
        ax.fill_between(np.linspace(vol_donahue[i], vol_donahue[i+1], num=100),
                    np.ones(100) * 2* plt_real.min(), np.ones(100) * 2 \
                    * plt_real.max(), alpha=0.4, color=colors[i])
    ax.fill_between(np.linspace(vol_donahue[3], 2* plt_real.max(), num=100),
        np.ones(100) * 2* plt_real.min(), np.ones(100) * 2 *plt_real.max(),\
    ...
    fig.savefig(f'data/plots/maccs_with_simpol/scatter_geckoq_donahue_classes.png')
