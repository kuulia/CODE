...
def main():
    ...
    geckoq_data['c'] = geckoq_data.apply(lambda x: log_psat_to_c(x['log_p_sat'], x['molar_mass'], temp_geckoq), axis=1)
    geckoq_data['volatility'] = geckoq_data.apply(lambda x: c_to_volatility_group(x['c']), axis=1)
    ...
    for seed in random_state:
        ...
        pred['c_pred'] = pred.apply(lambda x: log_psat_to_c(x['predictions'], x['molar_mass'], temp_geckoq), axis=1)
